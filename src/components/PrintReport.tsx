  ...
  return (
    <div className="hidden print:block" id="print-report">
      <header style={{ borderBottom: '2px solid #1A1D1A', paddingBottom: 10 }}>
        ...
        >
          PLANRAUM · Bebauungsplan-Konformitätsprüfung
        </div>
        <h1
          style={{
            fontFamily: 'Archivo, sans-serif',
        ...
        >
          ...
          {result.plan.municipality} · {result.plan.crs} · Stand {today}
        </div>
      </header>
      ...
      >
        ...
        >
          {summary.overall === 'PASS'
            ? 'Konform — all constraints satisfied'
            : summary.overall === 'FAIL'
              ? `Nicht konform — ${summary.fail} of ${summary.total} constraints violated`
              : `Prüfung nötig — ${summary.review} constraint(s) need review`}
        </span>
      </div>
      ...
      >
        <thead>
          <tr>
            {['Parameter', 'Allowed', 'Proposed', 'Verdict', 'Note'].map((h) => (
              <th
                key={h}
                style={{
                  textAlign: 'left',
      ...
      >
        Generated by PLANRAUM — AI Bebauungsplan reader. Demonstration output,
        nicht rechtsverbindlich. Values reflect human-reviewed extraction and the
        planner's proposed building.
      </footer>
    </div>
  )
}
